...
function NoteHtml(note, id) {
    ...
    return {
        ...
        dates: (size) => {
            var dates = document.createTextNode(this.note.created);
            return wrap("section", wrap("p", dates, "text-truncate"), size, "d-flex", "align-self-center");
        },
        controls: (size) => {
...
function createNotes() {
    var notesContainer = document.querySelector("#notes");
    Object.values(currentNotes).forEach((note, index) => {
        var id = "note__" + index;
